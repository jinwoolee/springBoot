CREATE TABLE USERS 
 ...
 COMMENT ON COLUMN USERID IS '사용자 아이디';
   COMMENT ON COLUMN USERNM IS '사용자 이름';
   COMMENT ON COLUMN PASS IS '비밀번호';
   COMMENT ON COLUMN REG_DT IS '사용자 등록일';
   COMMENT ON COLUMN ALIAS IS '별명';
   COMMENT ON COLUMN ADDR1 IS '주소';
   COMMENT ON COLUMN ADDR2 IS '상세주소';
   COMMENT ON COLUMN ZIPCODE IS '우편번호';
   COMMENT ON COLUMN FILENAME IS '업로드파일명';
   COMMENT ON COLUMN REALFILENAME IS '실제파일경로';
